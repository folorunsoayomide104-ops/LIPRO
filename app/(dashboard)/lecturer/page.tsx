            ...
            {courses.map((c) => (
              <Link key={c.id} href={`/courses/${c.id}`} className="block rounded-xl p-4 glass-hover">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold">{c.code} · {c.title}</div>
                    <div className="text-xs text-lipro-600/60 mt-0.5">{c.faculty} / {c.department} / Level {c.level} / {c.semester}</div>
                  </div>
