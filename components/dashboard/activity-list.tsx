...
// Deeper sage shades (not the light --primary itself) so the white icon on
// top stays readable — #b7d4ce is too light for white-on-top contrast.
const ICON_TONES = ['bg-[#4d766c]', 'bg-[#3f6459]', 'bg-emerald-500', 'bg-amber-500', 'bg-rose-500'];
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-studio-subtle">Your latest CBT attempts</p>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="grid h-8 w-8 place-items-center rounded-lg border border-studio-border bg-studio-elevated text-studio-muted">
      ...
      {attempts.length === 0 ? (
        <div className="rounded-xl border border-dashed border-studio-border-strong p-5 text-center">
          <Brain className="mx-auto h-6 w-6 text-studio-subtle" />
          <p className="mt-2 text-sm font-medium">No attempts yet</p>
          <p className="text-xs text-studio-subtle">Run your first CBT to see it here.</p>
        </div>
      ) : (
          ...
          {attempts.map((a, i) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-studio-subtle">{a.date}</div>
              </div>
              {a.pct !== null ? (
